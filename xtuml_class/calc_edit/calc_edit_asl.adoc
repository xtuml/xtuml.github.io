...
arranged in a __class diagram__.  A class may have a __state machine__
...
// Does the term, terminator, need more explanation?
...
// Should keep this simple by only suggesting one method.
// If you want to keep both methods, then the presentation needs to be made easier to read.
// A list might work better.
  This can be done by right-clicking on the ALU package and selecting
  link:06_New_Component.png[*New* > *Components* > *Component*],  or by 
  ...
  drawer of the link:08_Palette.png[*Palette*] and drawing the component by clicking the mouse cursor 
  on the editor canvas of the package.
...
. Establish a one-to-one association between the Calculator and Display classes
  ...
  *Association* widget.
  ...
  .. link:18_Drag_Association.png[Click inside the Calculator class, drag to
     inside the Display class] and drop.
...
. Create ports for the terminator and domain interfaces by opening the
  ...
  .. link:46_Rename_Port.png[Rename the ports] from their default names,
     'Port1' and 'Port2', to match the domain key letters, 'ALU' and 'disp'.
